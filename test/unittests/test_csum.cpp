...
TEST(misc, stress_csum) {
  ...
  for (long i = 0; i < 1000000; ++i) {
    ...
    const std::uint16_t
        updated_csum = IPChecksum::cfold(IPChecksum::diff16(old_prefix, raw, IPChecksum::cunfold(orig_csum)));
    ...
    ASSERT_EQ(updated_csum,verify_csum) << i << " size=" << size << " n=" << n << " orig=" << orig_csum << " updated=" << updated_csum
                << " verify=" << verify_csum << std::endl;
  }
}