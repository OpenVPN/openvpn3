...
TEST(Dns, ServerNoAddress)
{
  ...
  config.update_map();
  JY_EXPECT_THROW(DnsOptions dns(config), option_error, "dns server 0 does not have an address assigned");
}
...
TEST(Dns, ServerInvalidAddress)
{
  ...
  config.update_map();
  JY_EXPECT_THROW(DnsOptions dns(config), option_error, "dns server 0 invalid address: foobar");
}
...
{
  {
    ...
    config.update_map();
    JY_EXPECT_THROW(DnsOptions dns(config), option_error, "dns server 0 dnssec setting 'foo' invalid");
  }
  {
    OptionList config;
    config.parse_from_config("dns server 0 dnssec yes no\n", nullptr);
    config.update_map();
    JY_EXPECT_THROW(DnsOptions dns(config), option_error, "dns server 0 option 'dnssec' unknown or too many parameters");
  }
}
...
{
  {
    ...
    config.update_map();
    JY_EXPECT_THROW(DnsOptions dns(config), option_error, "dns server 0 transport 'avian-carrier' invalid");
  }
  {
    OptionList config;
    config.parse_from_config("dns server 0 transport DoT D'oh\n", nullptr);
    config.update_map();
    JY_EXPECT_THROW(DnsOptions dns(config), option_error, "dns server 0 option 'transport' unknown or too many parameters");
  }
}
...
{
  {
    ...
    config.parse_from_config(
      ...
      , nullptr);
    config.update_map();
    JY_EXPECT_THROW(DnsOptions dns(config), option_error, "dns server 0 cannot use exclude-domains and resolve-domains together");
  }
  {
    OptionList config;
    config.parse_from_config(
      ...
      , nullptr);
    config.update_map();
    JY_EXPECT_THROW(DnsOptions dns(config), option_error, "dns server 0 cannot use resolve-domains and exclude-domains together");
  }
}
