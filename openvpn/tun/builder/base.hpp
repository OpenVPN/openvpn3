...
namespace openvpn {
  ...
  {
  public:
    // Tun builder methods.  All methods returning bool use the return
    ...
    virtual bool tun_builder_add_address(const std::string& address, int prefix_length) { return false; }

    // Callback to add route to VPN interface
    virtual bool tun_builder_add_route(const std::string& address, int prefix_length) { return false; }

    // Callback to add DNS server to VPN interface
    virtual bool tun_builder_add_dns_server(const std::string& address) { return false; }
