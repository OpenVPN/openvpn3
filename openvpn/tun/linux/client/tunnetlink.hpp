...
struct TunMethods
{
                                  ...
                                  const unsigned int flags) // TunConfigFlags
    {
        ...
        if (!(flags & TunConfigFlags::DISABLE_REROUTE_GW))
        {
            ...
            {
                // add bypass route
                if ((flags & TunConfigFlags::ADD_BYPASS_ROUTES)
                    && !pull.remote_address.ipv6
                    && !(pull.reroute_gw.flags & RedirectGatewayFlags::RG_LOCAL))
                {
                    add_bypass_route(iface_name,
                                     pull.remote_address.address,
                                     false,
                                     rtvec,
                                     create,
                                     destroy);
