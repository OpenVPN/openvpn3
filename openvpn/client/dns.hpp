...
struct DnsServer
{
  enum class DomainType { Unset, Resolve, Exclude };
...
struct DnsOptionsMerger : public PushOptionsMerger
{
  ...
  struct DnsFilter : public OptionList::FilterBase
  {
    ...
    bool filter(const Option& opt) override
      {
	if (opt.empty() || opt.size() < 3 ||
	    opt.ref(0) != "dns" || opt.ref(1) != "server")
	  return true;
	  ...
	  if (o.size() >= 3 && o.ref(1) == "search-domains")
	      ...
	      else if (o.ref(3) == "resolve-domains")
		{
		  if (server.domain_type == DnsServer::DomainType::Exclude)
		    {
		      OPENVPN_THROW(option_error, "dns server " << priority
			    << " cannot use resolve-domains and exclude-domains together");
		    }
	      ...
	      else if (o.ref(3) == "exclude-domains")
		{
		  if (server.domain_type == DnsServer::DomainType::Resolve)
		    {
		      OPENVPN_THROW(option_error, "dns server " << priority
			    << " cannot use exclude-domains and resolve-domains together");
		    }
	      ...
	      else if (o.ref(3) == "dnssec" && o.size() == 5)
		{
		  if (o.ref(4) == "yes")
		    ...
		    {
		      OPENVPN_THROW(option_error, "dns server " << priority
			    << " dnssec setting '" << o.ref(4) << "' invalid");
		    }
		}
		...
		{
		  OPENVPN_THROW(option_error, "dns server " << priority << " option '"
			<< o.ref(3) << "' unknown or too many parameters");
		}
	    }
	  else
	    {
	      OPENVPN_THROW(option_error, "dns option unknown or invalid number of parameters "
