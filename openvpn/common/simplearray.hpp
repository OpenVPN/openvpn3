...
namespace openvpn {
   ...
   */
  template <typename T, template <typename> class Allocator = SimpleAllocator>
  class SimpleArray
  {
  public:
    ...
    explicit SimpleArray(const size_t size, const bool zero = false)
    {
      ...
      if (size)
	{
	  data_ = Allocator<T>::array_new(size);
	  if (zero)
    ...
    SimpleArray(const SimpleArray& other)
    {
      ...
      if (size_)
	{
	  data_ = Allocator<T>::array_new(size_);
    ...
    void erase()
    {
      if (data_)
	{
	  Allocator<T>::array_delete(data_, sizeof(T[size_]));
	  data_ = NULL;
	}
      size_ = 0;
    }
