                          ...
                          const size_t tailroom)
{
    ...
    auto dest = BufferAllocatedRc::Create(sizeof(std::uint32_t) + headroom + tailroom + LZ4_COMPRESSBOUND(src.size()), BufAllocFlags::NO_FLAGS);
                            ...
                            size_t max_decompressed_size = LZ4_MAX_INPUT_SIZE)
{
    ...
    // allocate dest buffer
    auto dest = BufferAllocatedRc::Create(headroom + tailroom + size, BufAllocFlags::NO_FLAGS);
    dest->init_headroom(headroom);
