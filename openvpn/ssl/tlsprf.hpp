...
namespace openvpn {
  ...
  private:
    ...
    static void hash (const typename CRYPTO_API::Digest& md,
		     ...
		     const size_t olen)
    {
      ...
      hash(CRYPTO_API::Digest::md5() ,S1,len,label,label_len,out1,olen);
      hash(CRYPTO_API::Digest::sha1(),S2,len,label,label_len,out2,olen);
