...
namespace openvpn {
  namespace IP {
	...
	try {
	  if (s2.empty())
	    {
	      const StringPair pair = split_by_char<StringPair, NullLex>(s1, '/');
	...
	return AddrMaskPair(); // NOTREACHED
      ...
      {
	try {
	  const StringPair pair = split_by_char<StringPair, NullLex>(s, '/');
	  return from_string_impl(pair, title);
	}
	catch (const std::exception& e)
	  {
	...
	if (pair.size() == 1 || pair.size() == 2)
	  {
		...
		if (is_number(pair[1].c_str()))
		  ret.netmask = Addr::netmask_from_prefix_len(ret.addr.version(),
							      parse_number<unsigned int>(pair[1].c_str()));
		else
